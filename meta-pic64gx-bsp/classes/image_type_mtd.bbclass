...
def make_mtdparts_shell_array(d):
    ...
    mtdparts = d.getVar('PIC64GX_MTDPARTS')

    if mtdparts is None:
        return "bbfatal 'PIC64GX_MTDPARTS is not defined.'"

    if len(mtdparts) == 0:
        return "bbfatal 'PIC64GX_MTDPARTS is empty.'"

    active_mtdid = d.getVar('PIC64GX_IS_ON_MTDID')

    if active_mtdid is None:
        return "bbfatal 'PIC64GX_IS_ON_MTDID is not defined.'"

    if len(active_mtdid) == 0:
        return "bbfatal 'PIC64GX_IS_ON_MTDID is empty. Please set it to the mtdid inside PIC64GX_MTDPARTS that you want to use.'"
    ...
    # Pick the mtdid that matches PIC64GX_IS_ON_MTDID.
    ...
    if current_mtdparts is None:
        return ("bbfatal 'Cannot find a valid mtdparts string inside PIC64GX_MTDPARTS (\"%s\"), "
                + "corresponding to PIC64GX_IS_ON_MTDID (\"%s\").'"
    ...
    for component in mtdparts.split(":")[1].split(","):
        ...
        if match is None:
            return "bbfatal \"'%s' is not a valid mtdparts string. Please set PIC64GX_MTDPARTS to a valid value\"" % mtdparts
...
flash_mtdpart() {
    ...
    # write.
    dd if="/dev/zero" \
        of="${IMGDEPLOYDIR}/${IMAGE_NAME}.${PIC64GX_MTD_TYPE}.mtdimg" \
        bs=1024 \
        seek=$kboffset \
        count=$kbsize \
        conv=notrunc
...
IMAGE_CMD:mtd () {
    ...
    ln -sfn "${IMAGE_NAME}.${PIC64GX_MTD_TYPE}.mtdimg" "${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.${PIC64GX_MTD_TYPE}.mtdimg"
